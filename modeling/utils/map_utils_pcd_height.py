...
from .baseline_utils import (
    project_pixels_to_world_coords,
    apply_color_to_map,
    pose_to_coords,
    convertInsSegToSSeg,
)
...
def find_neighborhood(agent_coords, occupancy_map):
    radius = (
        int(cfg.SENSOR.SENSOR_HEIGHT / cfg.SEM_MAP.CELL_SIZE)
    ...
    for idx in range(len(rr_cir)):
        ...
        mask_line = np.logical_and(
            np.logical_and(rr_line >= 0, rr_line < H),
        ...
        first_unknown = np.nonzero(
            occupancy_map[rr_line, cc_line] == cfg.FE.UNOBSERVED_VAL
        )[0]
        first_collision = np.nonzero(
            occupancy_map[rr_line, cc_line] == cfg.FE.COLLISION_VAL
        )[0]
        idx_first_unknown = 0 if len(first_unknown) == 0 else first_unknown[0]
        idx_first_collision = (
            len(rr_line) if len(first_collision) == 0 else first_collision[0]
        )
...
class SemanticMap:
    def __init__(self, split, scene_name, pose_range, coords_range, WH, ins2cat_dict):
        ...
        if True:
            ...
            gt_occupancy_map = np.load(
                f"{occ_map_path}/BEV_occupancy_map.npy", allow_pickle=True
            ).item()["occupancy"]
            gt_occupancy_map = np.where(
                gt_occupancy_map == 1, cfg.FE.FREE_VAL, gt_occupancy_map
            )  # free cell
            gt_occupancy_map = np.where(
                gt_occupancy_map == 0, cfg.FE.COLLISION_VAL, gt_occupancy_map
            )  # occupied cell
        ...
        self.neighborhood_history = np.zeros(
            (
                self.coords_range[3] + 1 - self.coords_range[1],
    ...
    def build_semantic_map(self, obs_list, pose_list, step=0, saved_folder=""):
        ...
        for idx, obs in enumerate(obs_list):
            ...
            mask_X = np.logical_and(
                xyz_points[0, :] > self.min_X, xyz_points[0, :] < self.max_X
            )
            ...
            x_coord = np.floor((xyz_points[0, :] - self.min_X) / self.cell_size).astype(
                int
            )
            y_coord = np.digitize(xyz_points[1, :], self.y_grid)
            z_coord = (self.H - 1) - np.floor(
                (xyz_points[2, :] - self.min_Z) / self.cell_size
            ).astype(int)
    ...
    def get_semantic_map(self):
        ...
        cells_in_occupied_range = smaller_four_dim_grid[
        ...
        mask_occupied = np.logical_and(
            cells_in_occupied_range >= cfg.SEM_MAP.POINTS_CNT, observed_area_flag
        )
    ...
    def get_observed_occupancy_map(self, agent_map_pose):
        ...
        # ============================= add current loc =========================
        occupancy_map[
            agent_coords[1] - 1 : agent_coords[1] + 2,
            agent_coords[0] - 1 : agent_coords[0] + 2,
        ] = cfg.FE.FREE_VAL
