...
def project_pixels_to_camera_coords(
    sseg_img,
    current_depth,
    current_pose,
    gap=2,
...
def project_pixels_to_world_coords(
    sseg_img,
    current_depth,
    current_pose,
    gap=2,
    ...
    good = np.logical_and(
        depth_points > cfg.SENSOR.DEPTH_MIN, depth_points < cfg.SENSOR.DEPTH_MAX
    )
...
def apply_color_to_map(semantic_map, dataset="MP3D"):
    ...
    if dataset == "MP3D":
        ...
        raise NotImplementedError(f"Dataset {dataset} not currently supported.")
...
def coords_to_pose(
    coords, map_data, cell_size=cfg.SEM_MAP.CELL_SIZE, flag_cropped=True
):
...
def pose_to_coords(
    cur_pose, map_data, cell_size=cfg.SEM_MAP.CELL_SIZE, flag_cropped=True
):
    """
    ...
    if flag_cropped:
        ...
        z_coord = floor((wh[0] - (tz - pose_range[1]) / cell_size) - coords_range[1])
    else:
