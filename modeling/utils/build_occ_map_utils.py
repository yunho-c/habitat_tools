...
class SemanticMap:
    ...
    def build_semantic_map(self, rgb_img, depth_img, sseg_img, pose, step_):
        ...
        if x_coord.shape[0] > 0:
            ...
            self.min_x_coord = min(
                max(np.min(x_coord) - self.map_boundary, 0), self.min_x_coord
            )
            self.max_x_coord = max(
                min(np.max(x_coord) + self.map_boundary, self.W - 1), self.max_x_coord
            )
            self.min_z_coord = min(
                max(np.min(z_coord) - self.map_boundary, 0), self.min_z_coord
            )
            self.max_z_coord = max(
                min(np.max(z_coord) + self.map_boundary, self.H - 1), self.max_z_coord
            )
    ...
    def get_semantic_map(self, step_):
        ...
        if smaller_four_dim_grid.shape[0] > 0 and smaller_four_dim_grid.shape[2] > 0:
            ...
            mask_occupied = np.logical_and(
                cells_in_occupied_range >= cfg.SEM_MAP.POINTS_CNT, mask_explored
            )
            mask_free = np.logical_and(mask_occupied == False, mask_explored)
            occupancy_map[mask_free] = cfg.FE.FREE_VAL
            occupancy_map[mask_occupied] = cfg.FE.COLLISION_VAL

            if occupancy_map.shape[0] > 0:
                save_occ_map_through_plt(
                    occupancy_map, f"{self.saved_folder}/step_{step_}_occ.jpg"
                )
    ...
    def save_final_map(self, ENLARGE_SIZE=5):
        ...
        if smaller_four_dim_grid.shape[0] > 0 and smaller_four_dim_grid.shape[2] > 0:
            ...
            occupancy_map = cv2.resize(
                occupancy_map,
                (
                    int(occupancy_map.shape[1] * ENLARGE_SIZE),
                    int(occupancy_map.shape[0] * ENLARGE_SIZE),
                ),
                interpolation=cv2.INTER_NEAREST,
            )
            save_occ_map_through_plt(
                occupancy_map, f"{self.saved_folder}/final_occupancy_map.jpg"
            )
